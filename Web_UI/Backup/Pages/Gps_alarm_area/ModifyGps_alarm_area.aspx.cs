...
{
	/// <summary>
	/// ModifyGps_alarm_area 的摘要说明。
	/// </summary>
	public class ModifyGps_alarm_area : Page
	{
		...
		private void BindData()
		{
			...
			try
			{
				...
				Gps_alarm_areaInfo gps_alarm_areaInfo = gps_alarm_area.FindByID(int.Parse(Request.QueryString["ID"])) as Gps_alarm_areaInfo;
				if (gps_alarm_areaInfo != null)
				{
					ListItem item = null;
		    		txtAlarm_id.Text = gps_alarm_areaInfo.Alarm_id.ToString();
 		    		txtArea_name.Text = gps_alarm_areaInfo.Area_name.ToString();
 		    		txtAlarm_type.Text = gps_alarm_areaInfo.Alarm_type.ToString();
 		    		txtStart_time.Text = gps_alarm_areaInfo.Start_time.ToString();
 		    		txtEnd_time.Text = gps_alarm_areaInfo.End_time.ToString();
 		    		txtIs_send_sms.Text = gps_alarm_areaInfo.Is_send_sms.ToString();
 		    		txtArea_department.Text = gps_alarm_areaInfo.Area_department.ToString();
 		    		txtArea_entri.Text = gps_alarm_areaInfo.Area_entri.ToString();
 		    		txtArea_geomertry_type.Text = gps_alarm_areaInfo.Area_geomertry_type.ToString();
 		    		txtArea_linecolor.Text = gps_alarm_areaInfo.Area_linecolor.ToString();
 		    		txtArea_linewidth.Text = gps_alarm_areaInfo.Area_linewidth.ToString();
 		    		txtArea_data.Text = gps_alarm_areaInfo.Area_data.ToString();
  				}
			}
			catch (Exception ex)
			{
				...
				if (gps_alarm_areaInfo != null)
				{
	    			gps_alarm_areaInfo.Alarm_id = Convert.ToDecimal(txtAlarm_id.Text);
 	    			...
 	    			gps_alarm_areaInfo.Area_data = Convert.ToByte[](txtArea_data.Text);
  					gps_alarm_area.Update(gps_alarm_areaInfo);
				 }
				*/
			}
			catch (Exception ex)
