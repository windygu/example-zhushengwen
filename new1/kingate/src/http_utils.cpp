...
bool CheckStringModel(const char *str,const char *model)
{
	...
	if(model[i]=='*'){
	  ...
	  for(int p=j;p<length2;p++){
	    if(str[p]==model[i+1]){
	      if(CheckStringModel(str+p,model+i+1))
		return true;
	    }
	  }
...
bool CheckFileModel(const url  *m_url,const char *model)
{
	...
	if(!m_url->proto || !m_url->host || !m_url->path)
		return false;
	int u_len=strlen(m_url->proto)+strlen(m_url->host)+strlen(m_url->path)+10;
	...
	if(strcasecmp(m_url->proto,"https")==0)
		http_ssl=true;
	snprintf(url_file,u_len-1,"%s://%s:%d%s",m_url->proto,m_url->host,m_url->port,m_url->path);
	ret=CheckStringModel(url_file,model);
	free(url_file);
	return ret;
}
