...
public:
    ...
    char* GetDefaultKingateConf(){ return (char*)szDefaultKingateConf;}
    char* GetDefaultAccessConf(){ return (char*)szDefaultAccessConf;}
private:
	bool KConfig::GetList(const char *filename);
	int skip_next_line(const char *str);
	int get_word(const char *str,char *word,bool &multi);
...
private:
	...
	bool name_multi;
	bool value_multi;
    static unsigned char szDefaultKingateConf[];
    static unsigned char szDefaultAccessConf[];
};
#endif
