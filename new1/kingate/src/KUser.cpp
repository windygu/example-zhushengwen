...
bool KUser::LoadUser()
{
	...
	if(strlen(conf.user_model)==0){
		...
		snprintf(tmp,sizeof(tmp)-1,"%s%s",conf.path,USER_FILE);
...
#ifdef HAVE_DLFCN_H
		memset(tmp,0,sizeof(tmp));
		snprintf(tmp,sizeof(tmp)-1,"%s%s",conf.path,conf.user_model);
...
bool KUser::CheckGroup(unsigned uid,unsigned gid)
{
	...
	m_root_lock.Lock();
	it=m_all_user2.find(gid);
	if(gid!=ALLUSER && it==m_all_user2.end()){
	//	printf("gid is err.\n");
		goto err;
	}
	if(uid==gid)
