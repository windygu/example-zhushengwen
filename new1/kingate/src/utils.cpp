...
int udp_encode(mysocket * tcp_sock,mysocket *udp_sock,const char *src_data,int length_src_data,socks_udp *m_socks_udp)
{
	...
	if(length_src_data>PACKAGE_SIZE&&length_src_data<=0)
		return 0;
	if(allow_connect(SOCKS,tcp_sock,udp_sock->get_client_name(),udp_sock->get_client_port(),dst_ip,false,m_socks_udp->uid)==DENY)
		return 0;
	...
	convert_addr(udp_sock->get_client_name(),dest_data+4);
	dest_data[8]=udp_sock->get_port()/0x100;
	dest_data[9]=udp_sock->get_port()-(src_data[8]&0xff)*0x100;
...
//	printf("srcuid=%d,dstuid=%d.\n",tcp_sock->uid,udp_sock->uid);
	...
	if(m_socks_udp->client_ip==0){//it is first data;
		m_socks_udp->client_ip=udp_sock->get_client_addr();
		m_socks_udp->port=udp_sock->get_client_port();
	}
...
int create_select_pipe(mysocket * server,mysocket *client,int tmo,int max_server_len,int max_client_len)
{
	struct timeval tm;
	fd_set readfds;
	int maxfd=0;
	...
	int s1=server->get_socket(),s2=client->get_socket();
...
//	printf("max_server_len=%d,max_client_len=%d\n",max_server_len,max_client_len);
	for(;;){
		...
		if(FD_ISSET(s1,&readfds)){
			...
			if((len=server->recv_t(buf,max_recv_len,0))<=0){
				ret=s1;
				break;
			}
	//		buf[len]=0;
	//		printf("***************leave read:\n%s\n",buf);
			...
			if((len=client->recv_t(buf,max_recv_len,0))<=0){
				ret=s2;
				break;
			}
	//		buf[len]=0;
...
//	struct timeval * m_timeout=NULL;
	int length=0;
	int s1=server->get_socket(),s2=client->get_socket(),ret,ftp_data_cmd=0,model=m_server->model;
	fd_set readfds;
...
/******************** server 有数据可以读 ****************************/
	//	printf("have msg come in.\n");
		if(FD_ISSET(s1,&readfds)){
				if((length=server->recv_t(str,PACKAGE_SIZE,0))<=0)
					break;				
		//		printf("s:\n");
		//		PRINT(str,length);
		//		str[length]=0;
...
/******************** client 有数据可以读 **********************/
		if(FD_ISSET(s2,&readfds)){
			if((length=client->recv_t(str,PACKAGE_SIZE,0))<=0)
				break;
		//	str[length]=0;
		//	printf("c:\n%s",str);
		//	printf("c:\n");
		//	PRINT(str,length);
			switch (model){
				case MODEL_SOCKS_UDP:
					if(client->get_client_addr()==server->get_client_addr())
					{
...
FUNC_TYPE FUNC_CALL server_thread(void *m_mysocket)
{
	...
	unsigned long ip=m_server->server->get_client_addr();
...
//	goto cleanup;
	...
	if(allow_connect(m_server->model,m_server->server,NULL,0,uid)==DENY)
		goto cleanup;
	#endif
...
cleanup:
	...
	m_server->server->get_addr(),
	m_server->server->get_port(),
	server_port,
...
//	pthread_mutex_init(&filter_time_lock,NULL);
	...
	//		printf("select error.maybe recv a signal.\n");
		...
		if((tmp->m_server.accept_fd=tmp->server.accept())<=0){
		...
		klog(DEBUG_LOG,"client %s:%d come in\n",tmp->server.get_addr(),tmp->server.get_port());
		...
		if(m_thread_p.Start(m2_server)<=0){
			delete m2_server;
			tmp->server.use(OLD);
			continue;
		}
...
int get_path(char *argv0,char *path)
{
	#ifndef _WIN32
	char sep='/';
	#else
