...
//	stringstream s;
	...
	if(item=="service"){
		...
		for(i=TOTAL_SERVICE-1;i>=0;i--){
			...
			s << service_name[i] << " time out:</td><td>" << "<input type=text name=" << time_out << " value=" << m_config.GetValue(time_out.c_str()) << "></td></tr>\n";
		}
		s << "</table>";
		...
		s << "flag:<input type=checkbox checked name=flag value=proxy>proxy<br>\n";
	...
	//		}while( (tmp_redirect=tmp_redirect->next)!=NULL );
		...
		s << "mem_min_cache:" << "<input type=text name=mem_min_cache value=" << m_config.GetValue("mem_min_cache") <<"><br>";
		s << "mem_max_cache:" << "<input type=text name=mem_max_cache value=" << m_config.GetValue("mem_max_cache") <<"><br>";
		s << "disk_min_cache:" << "<input type=text name=disk_min_cache value=" << m_config.GetValue("disk_min_cache") <<"><br>";
		s << "disk_max_cache:" << "<input type=text name=disk_max_cache value=" << m_config.GetValue("disk_max_cache") <<"><br>";
		s << "min refresh time:<input type=text name=refresh_time value=" << conf.refresh_time << "><br>";
		...
		s << "</select><br>";
		s << "log the connection close msg:<select name=log_close_msg><option value=on ";
		...
		s << "total max threads(0 for unlimited): <input type=text name=max value=" << m_config.GetValue("max") <<"><br>";
		s << "total max threads for each ip(0 for unlimited): <input type=text name=max_per_ip value=" << m_config.GetValue("max_per_ip") << "><br>";
		s << "min free thread for close the free thread: <input type=text name=min_free_thread value=" << conf.min_free_thread << "><br>";
		s << "user active time out:<input type=text name=user_time_out value=" << conf.user_time_out << "><br>";
...
#endif
		s << "http accelerate :<select name=http_accelerate><option value=on ";
		...
		s << "insert x_forwarded_for in http proxy :<select name=x_forwarded_for><option value=on ";
		...
		s << "insert via in http proxy :<select name=insert_via><option value=on ";
		if(conf.insert_via)
			s << "selected";
		s << " >on</option><option value=off ";
		if(!conf.insert_via)
		...
		s << "socks5 support user auth:<select name=socks5_user><option value=on ";
		if(conf.socks5_user)
			s << "selected";
		s << " >on</option><option value=off ";
		if(!conf.socks5_user)
...
bool KHttpManage::configsubmit()
{
	...
	if(item=="service"){
		...
		if(action=="delete"){
			...
			http_redirect_value << " " << getUrlValue("flag") << "|";
			m_config.AddValue("http_redirect",http_redirect_value.str().c_str(),false,true,true);
			
		}
	}else if(item=="cache"){
		m_config.SetValue("mem_min_cache",getUrlValue("mem_min_cache").c_str());
		m_config.SetValue("mem_max_cache",getUrlValue("mem_max_cache").c_str());
		m_config.SetValue("disk_min_cache",getUrlValue("disk_min_cache").c_str());
		m_config.SetValue("disk_max_cache",getUrlValue("disk_max_cache").c_str());
		...
		conf.mem_min_cache=get_cache(getUrlValue("mem_min_cache").c_str());
		conf.mem_max_cache=get_cache(getUrlValue("mem_max_cache").c_str());
		conf.disk_min_cache=get_cache(getUrlValue("disk_min_cache").c_str());
		conf.disk_max_cache=get_cache(getUrlValue("disk_max_cache").c_str());
...
{	
	...
	stringstream s;
        ...
        if(TEST(rq->flags,RQ_HAS_KEEP_CONNECTION) && (rq->meth==METH_GET))
                s << "keep-alive\r\n";
        else{
		return_result=false;
                s << "close\r\n";
...
bool KHttpManage::sendMainPage()
{
	...
	#ifndef DISABLE_USER
	s << user_info.user->user.c_str() << " login from " << rq->server->get_client_name() << " total time(sec) " << user_info.user->total_time+(time(NULL)-user_info.login_time) << " total send size:" << user_info.user->send_size/1024 << "kb total recv size:" << user_info.user->recv_size/1024 << "kb";
...
//	return false;
	...
	if(strcmp(rq->url.path,"/login")==0){		
		if(m_user.CheckPasswd(getUrlValue("user").c_str(),getUrlValue("password").c_str())>0){
			m_user.Login(rq->server->get_client_addr(),getUrlValue("user").c_str());
			goto continue_check;	
		}else{
	//		sendTest();
...
continue_check:
	#ifndef DISABLE_USER
	if(m_user.CheckLogin(rq->server->get_client_addr(),&user_info)==0){
		return sendLogin();
		
	}
	if(strcmp(rq->url.path,"/logout")==0){
		m_user.Logout(rq->server->get_client_addr());
...
//		sendHeader(200);
	...
	if(strcmp(rq->url.path,"/info")==0){
		stringstream s;
//		sendHeader(200);
		int total_mem_size=0,total_disk_size=0;
		int total_run_time=time(NULL)-kingate_start_time;
		get_cache_size(&total_mem_size,&total_disk_size);
		s << "<html><head><title>kingate(" << VER_ID << ") infomation</title><META content=\"5; URL=/info\" http-equiv=Refresh></head><body>";
		...
		s << "<tr><td>free thread</td><td>" << m_thread_p.getFreeThread()<< "</td></tr>\n";
		//s << "<tr><td>mem cache size</td><td>" << 
		s << "</table>";
		s << "<h3>Connection Infomation</h3><table><tr><td>src_ip</td><td>service|port</td><td>dst_ip</td><td>dst_port</td></tr>";
		s << getConnectionInfo() << "</table>";
		s << "</body></html>";
		return sendHttp(s.str());
