...
//	char proxy_connection[20];
	...
	if(client.connect(rq->url.dst_ip,rq->url.dst_port)<=0){
...
//	printf("%s",buffer);
	...
	if(build_data_connect(&client,&data_client,err_msg)<=0)
		goto clean;
...
//		m_server.time_out=30;
		create_select_pipe(server,&data_client,30);
		client.send("QUIT\r\n");
		return;
