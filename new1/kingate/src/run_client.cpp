...
//socklen_t		mysalen = sizeof(rq.my_sa);
//struct	group		*group;
//int			miss_denied = TRUE;
int i=0;
//int			so;//,redir_mods_visited, auth_mods_visited;
//int			accepted_so;
mysocket client;
...
//		rq.so = so;
		rq.server=server;
		rq.client=&client;
	//	memcpy(&rq.client_sa, work->server->get_sockaddr(), sizeof(rq.client_sa));
	//	getsockname(so, (struct sockaddr*)&rq.my_sa, &mysalen);
		...
		ip = buf;
		forever() {
			got=server->recv(cp,current_size-(cp-ip));
					if(got<=0){
		...
		#endif
		if( (work->model==MANAGE) || (strcmp(server->get_server_name(),rq.url.host)==0 && conf.port[MANAGE]==rq.url.port)){
			model=MANAGE;
		}else{
			model=HTTP;
		}
		...
		if(rq.proto==PROTO_HTTP){
			for(i=0;i<conf.m_http_redirects.size();i++){// check http redirect
			  	if(MatchIPModel(conf.m_http_redirects[i].dst,rq.url.dst_ip,rq.url.port) &&
		    	 	( (conf.m_http_redirects[i].file_ext==NULL) || (CheckFileModel(&rq.url,conf.m_http_redirects[i].file_ext)))              ){
		   	 	...
		   	 	if(TEST(conf.m_http_redirects[i].flag,USE_LOG))
		      			klog(MODEL_LOG,"[HTTP]%s:%d request http://%s:%d%s redirect to %s:%d\n",rq.server->get_client_name(),rq.server->get_client_port(),rq.url.host,rq.url.port,rq.url.path,make_ip(conf.m_http_redirects[i].hosts[index].ip).c_str(),conf.m_http_redirects[i].hosts[index].port);
		    		break;
		  		}
		    	} 
		}
...
destroy_obj(struct mem_obj *obj,int in_hash)
{
		...
		u_short url_hash = S_hash(&obj->url);
...
//printf("src=%s\n",src);
    ...
    *p = ' ';
    ...
    httpv--;
    *httpv = 0;
    if ( parse_url(p, httpv+1, &rq->url,rq) ) {
		*httpv = ' ';
		return(-1);
    }
  //  printf("path=%s.\n",rq->url.path);
...
int
parse_url(char *src, char *httpv, struct url *url, struct request *rq)
{
...
normal:;
	...
	if ( (pval=atoi(number)) != 0 )
		...
		say_bad_request( "Bad port value:", number,
			ERR_BAD_PORT,rq);
		IF_FREE(login);
		IF_FREE(password);
		xfree(proto);
 ...
 */
void
say_bad_request(char* reason, char *r, int code, struct request *rq)
{
...
int is_rediect(char* str)
{
	...
	//	MessageBox(NULL,(LPCSTR)"你的使用已经过期了！",(LPCSTR)"使用过期了",MB_OKCANCEL);
	//		
...
{
string	hdr = "HTTP/1.0 302 Found\r\nLocation: http://";
hdr=hdr+direct_url;
hdr=hdr+"\r\nConent-Type: text/html; charset=UTF-8\r\nContent-Length: 0\r\n\r\n";
if(rq!=NULL)
		rq->server->send(hdr.c_str(),hdr.length());
return; 
}
...
{
//	mysocket client;
	klog(MODEL_LOG,"[HTTPS]%s:%d connect https://%s:%d\n",rq->server->get_client_name(),rq->server->get_client_port(),rq->url.host,rq->url.port);
...
bool http_start(request *rq)//http代理
{
	...
	*/	
	...
	if(rq->url.port==80){
		klog(MODEL_LOG,"[HTTP]%s:%d %s get http://%s%s %d %d\n",rq->server->get_client_name(),rq->server->get_client_port(),msg,rq->url.host,rq->url.path,(time(NULL)-start_time),head_status);
	}else{
		klog(MODEL_LOG,"[HTTP]%s:%d %s get http://%s:%d%s %d %d\n",rq->server->get_client_name(),rq->server->get_client_port(),msg,rq->url.host,rq->url.port,rq->url.path,(time(NULL)-start_time),head_status);
	}
	return result;
}
#endif
