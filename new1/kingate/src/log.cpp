...
void klog_open_file()
{
	...
	if(m_fp!=NULL)
		return;
	char *file_name=(char *)malloc(strlen(conf.path)+17);
	strcpy(file_name,conf.path);
...
{
	struct tm * tm;
	char *new_file=(char *)malloc(strlen(conf.path)+1+2*17+32);
	if(new_file==NULL)
		return;
	char *old_file=(char *)malloc(strlen(conf.path)+16+1);
	if(old_file==NULL){
		free(new_file);
		return;
	}
	...
	now_tm=time(NULL);
	tm=localtime(&now_tm);
	sprintf(old_file,"%s/var/kingate.log",conf.path);
	sprintf(new_file,"%s/var/kingate.log%02d%02d_%02d%02d%02d\0",conf.path,tm->tm_mon+1,tm->tm_mday,tm->tm_hour,tm->tm_min,tm->tm_sec);
	pthread_mutex_lock(&log_lock);
	klog_close_file();
//	system(cmd);	
