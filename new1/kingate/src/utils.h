...
struct _SERVER{
//	unsigned int port;			//监听端口
//	unsigned  int max;			//最大连接数
//	unsigned  int time_out;
	int		accept_fd;
//	filter * m_filter;			//过滤系统
	short model;				//工作模式。HTTP,FTP,...
//	int connect_time;
//	mysocket * (* create_connect )( struct _SERVER *);//建立与远程主机连接的函数入口
...
int create_select_pipe(mysocket * server,mysocket *client,int tmo=0,int max_server_len=-1,int max_client_len=-1);
...
int get_path(char *argv,char *path);
