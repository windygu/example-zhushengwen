...
////		PRINT(str,length);
	...
	//	printf("passwd_len=%d.",passwd_len);
		...
		if((uid=m_user.CheckPasswd(user,passwd))==0){
			str[1]=1;
			server->send(str,2);
			klog(ERR_LOG,"user=%s.passwd=%s. is error from %s:%d\n",user,passwd,server->get_client_name(),server->get_client_port());
			goto cleanup;
		}
skip_check:
		str[0]=1;
...
//		printf("uid=%d.\n",uid);
	...
	klog(MODEL_LOG,"socks5 client %s:%d connect host %s:%d\n",server->get_addr(),server->get_port(),host,port);
	convert_addr(server->getsockname(),str+4);
	if(str[1]==1){
		if(client->connect(host,port)<=0){
			str[1]=1;
		...
		if(is_local_ip(client))
			goto cleanup;
		str[1]=0;str[3]=1;
		str[8]=client->get_server_port()/0x100;
		str[9]=client->get_server_port()-str[8]*0x100;
		server->send(str,10);		
