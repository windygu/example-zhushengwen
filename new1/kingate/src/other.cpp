...
//	printf("real_user=%s,host=%s,port=%d\n",real_user,host,port);
	...
	client->set_time(CONNECT_TIME_OUT);
...
//	printf("recv remote reply:%s\n",buf);
	...
	klog(MODEL_LOG,"[SMTP] %s:%d connect host %s:%d success\n",m_server->server->get_client_name(),m_server->server->get_client_port(),host,port);
done:
	free(buf);
	if(host)
		free(host);
...
clean:
	free(buf);
	klog(MODEL_LOG,"[SMTP] %s:%d use smtp proxy error\n",m_server->server->get_client_name(),m_server->server->get_client_port());
	if(host)
		free(host);
	if(user)
		free(user);
...
//	printf("str=%s,msg=%s,host=%s.\n",str,msg,host);
	...
	server->set_time();
	client->set_time();
	klog(MODEL_LOG,"[POP3] %s:%d connect host %s:%d sucess\n",server->get_client_name(),server->get_client_port(),host,port);
	return client;
...
clean:
	delete client;
	klog(MODEL_LOG,"[MMS] %s:%d cann't connect host %s:%d",server->get_client_name(),server->get_client_port(),host,port);
	return NULL;
}
#endif
#ifndef DISABLE_RTSP
