...
namespace _58
{
    ...
    public class HttpHelper
    {
        ...
        private void SetCookie(HttpItem objhttpItem)
        {
            ...
            if (objhttpItem.Cc != null)
            {
                request.CookieContainer = objhttpItem.Cc;
            }

            //request.CookieContainer = cc;
            //chkCookie(cc, objhttpItem);

        }
        private void chkCookie(CookieContainer cc, HttpItem objhttpItem)
        {
            if (cc.Count > 0)
            {
                cool = cc.GetCookies(new Uri(objhttpItem.URL))[0].Value;
                if (objhttpItem.CookieCollection != null)
                {
                    oldcool = objhttpItem.CookieCollection[0].Value;
                }
                else if (newcool.Contains(oldcool) && oldcool.Length > 0 && newcool.Length > 0)
                {
                    // throw new WebException("cookie是用户传来的上一次内容", WebExceptionStatus.UnknownError);
                    System.Windows.Forms.MessageBox.Show("cookie是用户传来的上一次内容");
                    return;
                    //throw new Exception("cookie是用户传来的上一次内容");

                }
                else if (newcool.Contains(cool) && cool.Length > 0 && newcool.Length > 0)
                {
                    System.Windows.Forms.MessageBox.Show("本次请求的cookie与上次相同");
                    return;
                    //throw new WebException("本次请求的cookie与上次相同");
                }
                else
                {
                    oldcool = cool;
